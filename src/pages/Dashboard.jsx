...
export default function Dashboard() {
  ...
  useEffect(() => { if (!user) navigate('/login'); else loadUsage() }, [user])
...
Cada hook: máximo 3 segundos al leerlo. Generan curiosidad o miedo a perderse algo.
Formato: 1. hook  2. hook  etc. Solo en español, sin explicaciones.`,
...
Solo los hashtags con #, separados por espacio.`
    ...
    try {
      ...
      const text = data.content?.[0]?.text || 'Error al generar. Intenta de nuevo.'
      setResult(text)
  ...
  const remaining = isPro ? '∞' : `${DAILY_FREE_LIMIT - usageCount}/${DAILY_FREE_LIMIT}`
    ...
    <div style={{ background: '#08080F', minHeight: '100vh', fontFamily: "'Courier New', monospace" }}>
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 28px', borderBottom: '1px solid #1A1A2E' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <span style={{ fontSize: '12px', color: '#00FFB2', background: '#00FFB215', border: '1px solid #00FFB230', padding: '3px 10px', borderRadius: '2px' }}>
            {remaining} generaciones
          </span>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '36px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px', marginBottom: '28px' }}>
          {MODES.map(m => (
            <button key={m.id} onClick={() => setMode(m.id)} style={{
              ...
              borderRadius: '5px', padding: '14px 10px', cursor: 'pointer', textAlign: 'center'
            }}>
              <div style={{ fontSize: '14px', fontWeight: '700', color: mode === m.id ? '#00FFB2' : '#FFF' }}>{m.label}</div>
              <div style={{ fontSize: '11px', color: '#555', marginTop: '4px' }}>{m.desc}</div>
            </button>
          ))}
        </div>

        {/* FORM */}
        <div style={{ background: '#0F0F1A', border: '1px solid #1E1E30', borderRadius: '6px', padding: '24px', marginBottom: '20px' }}>
          ...
          <button onClick={generate} disabled={loading} style={{
            width: '100%', background: loading ? '#00FFB260' : '#00FFB2',
        ...
        {/* RESULT */}
        {result && (
          <div style={{ background: '#0F0F1A', border: '1px solid #00FFB230', borderRadius: '6px', padding: '24px' }}>
            ...
            <pre style={{ color: '#D0D0E0', fontSize: '14px', lineHeight: '1.8', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{result}</pre>
          </div>
        )}

        {/* UPGRADE BANNER */}
        {!isPro && (
          <div style={{ marginTop: '20px', background: '#FF6B3510', border: '1px solid #FF6B3530', borderRadius: '6px', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <span style={{ color: '#FF6B35', fontWeight: '700', fontSize: '13px' }}>⚡ Pro — Scripts ilimitados</span>
              <p style={{ color: '#666', fontSize: '12px', marginTop: '4px' }}>Sin límite diario + tonos personalizados + exportar</p>
            </div>
            <button style={{ background: '#FF6B35', color: '#000', border: 'none', padding: '10px 20px', borderRadius: '3px', cursor: 'pointer', fontWeight: '700', fontSize: '13px', fontFamily: "'Courier New', monospace" }}>
              $9.99/mes
            </button>
          </div>
        )}
...
const inputStyle  = { background: '#08080F', border: '1px solid #2A2A3E', color: '#E0E0E0', padding: '10px 14px', borderRadius: '4px', fontSize: '13px', fontFamily: "'Courier New', monospace", outline: 'none', width: '100%' }
