    ...
    <div style={pageStyle}>
      <div style={cardStyle}>
        ...
        <p style={{ textAlign: 'center', color: '#555', fontSize: '13px', marginBottom: '24px' }}>
          Gratis. Sin tarjeta. 3 scripts/día.
        </p>
        {error && <div style={errorStyle}>{error}</div>}
        {msg && <div style={successStyle}>{msg}</div>}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <input style={inputStyle} type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} />
          <input style={inputStyle} type="password" placeholder="Contraseña (mín. 6 chars)" value={password} onChange={e => setPassword(e.target.value)} />
